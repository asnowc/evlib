...
async function build(opts: RollupOptions) {
    const { output: outputOptions = {} } = opts;

    const output = outputOptions instanceof Array ? outputOptions : [outputOptions];

    let bundle: RollupBuild | undefined;
    let buildFailed = false;
    try {
        // 启动一次打包
        bundle = await rollup(opts);
...
async function generateOutputs(bundle: RollupBuild, outputOptionList: OutputOptions[]) {
