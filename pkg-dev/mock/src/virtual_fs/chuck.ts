...
export abstract class Chuck {
  ...
  getChunkFromPath(path: string): Chuck | undefined {
    ...
    if (root !== "") return this.root?.getChildChuck(Path.relative(root, path));
      ...
      if (this instanceof Dir) return this.getChildChuck(path);
        ...
        path = Path.join(path);
        if (path.startsWith(".."))
          return this.parent?.getChildChuck(path.slice(3));
        return;
      }
    }
  }
  stat(): Stats {
    return new Chuck.Stats(
      { ...this.statsTime, ...this.staticStats },
      this,
    ) as any as Stats;
  }
  get path() {
  ...
  constructor(
    protected readonly metadata: Readonly<Metadata>,
    staticStats?: Partial<StatsBase>,
  ) {
...
export class Dir extends Chuck {
  ...
  constructor(
    metadata: Metadata,
    info: Readonly<DirInfo> | Readonly<DirInfoContent>,
  ) {
    let dirInfo: Readonly<DirInfo>;
    if (!info["*type"])
      dirInfo = { "*type": ChunkType.dir, content: info as DirInfoContent };
...
export class File extends Chuck {
  ...
  constructor(metadata: Metadata, info?: FileInfo | FileInfoContent | File) {
    ...
    if (info instanceof File) content = info.content;
      ...
      if (typeof info === "string" || info instanceof Buffer)
        content = Buffer.from(info);
