 ...
 */
err.argType = function argTypeError(
  errorArgs: any,
  correctTypeDesc: ArgDesc,
  index: number = 0,
): CommonError {
  if (index >= 0) {
...
err.arg = function arg(arg: any, msg: string, index: number = 0): CommonError {
  ...
  if (index >= 0) {
    ...
    var re = new CommonError(message, {
      name: "函数参数错误",
      up: 1,
      slice: 1,
    });
...
err.inArg = function inArgError(
  ...
  if (index1 >= 0) {
    if (typeof index2 === "number") {
      ...
      throw err.argType(index2, "number|string", 3);
