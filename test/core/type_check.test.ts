...
import { checkType, ExceptType, InferExcept, typeChecker } from "evlib";
...
describe("内置测试函数", function () {
  it("联合类型", function () {
    const res = checkType(
      { s: 3, i: null },
      {
        ...
        i: typeChecker.union(["string", (a: any) => undefined]),
      },
    );
    //联合类型
    expect(res).toCheckPass();
  ...
  describe("可选", function () {
    describe("自定义可选", function () {
      ...
      it("错误的可选", function () {
        expect(
          checkType({ q: { c: 8 } }, { q: optional({ c: "string" }) }),
  ...
  describe("数组类型判断", function () {
    ...
    it("数组长度限制", function () {
      ...
      res = checkType([2, 4, "d", 78], typeChecker.arrayType("number", 3), {
        checkAll: true,
      });
...
describe("自定义校验函数", function () {
  ...
  test("record属性替换", function () {
    ...
    const { value, error } = checkType(
      obj,
      typeChecker.record((val) => ({ value: val / 2, replace: true })),
    );
    expect(value.aa, "值已被替换成").toBe(5);
    expect(error).toBeUndefined();
  });
  test("数组替换", function () {
    ...
    const { value, error } = checkType(
      obj,
      typeChecker.arrayType((val: any) => ({ value: val / 2, replace: true })),
    );
