...
function add(json, faNode) {
  ...
  if (Object.typeof(json, false) === "object") {
    ...
    if (typeof nodeName === "string")
      ...
      var err = new Error("参数1中的nodeName属性必须我为string类型")
        .prior()
        .add({ nodeName });
      err.Nty = 2;
      throw err;
  ...
  if (faNode instanceof HTMLElement) {
    try {
      ...
      var err = new Error("无法添加到父节点:(" + error.message + ")")
        .prior()
        .add(faNode);
      err.Nty = 5;
      throw err;
    ...
    var err = new Error("参数2必须为节点，否则无法添加到父节点")
      .add(faNode)
      .prior();
    err.Nty = 4;
    throw err;
  ...
  if (typeof json.onload === "function") {
    //load事件
    try {
      json.onload.call(newNode);
    } catch (error) {
      error.prior().message =
        "参数1的onload函数被执行时出现异常：(" + error.message + ")";
...
function set(json, node) {
  ...
  for (var attName in json) {
    ...
    if (attName === "style") {
      //样式属性
      if (type !== "Object") {
        var err = Error("参数1中的style属性必须为object类型")
          .prior()
          .add({ style: value });
        err.Nty = 4;
        throw err;
      ...
      if (type !== "Array") {
        var err = new Error("参数1中的children属性必须为Array类型")
          .prior()
          .add({ children: value });
        err.Nty = 1;
        throw err;
...
function addChilds(node, childs) {
  ...
  try {
    for (var i = 0; i < childs.length; i++) {
      if (childs[i] instanceof HTMLElement) node.appendChild(childs[i]);
      else GUI.add(childs[i], node);
    }
  } catch (error) {
    if (error.Nty === 1) {
      var adObj = { children: error.addInfo };
