...
import { err } from "../std/err.js";
type propDescription = Parameters<typeof Object.defineProperty>[2];
/** 参数检查装饰器类型 */
type argTestDecorator = (tagClass: any, fxName: string, descrp: propDescription) => any;

export const ops = {
    /** 是否开启debug模式 */
    debug: false,
    /** error对象的messag属性, 每次抛出异常会调用这个函数生成message字符串 */
    createErrMsg(args: IArguments, index: number, ops: ReturnType<checker["testFx"]>) {
        var cType = ops?.coType ?? typeof type;
        var nType = ops?.nowType ?? typeof arguments[index];
        var msg = "参数" + (index + 1) + "[" + cType + "]错误: 当前传入类型[" + nType + "]";
        return msg;
    },
};
/** 自定义检测器选项 */
interface checker {
    ...
    testFx: (
        args: IArguments,
        index: number
    ) => void | { coType: string; nowType?: string; binding?: any; childMsg?: string };
...
export function typeAll(type: argType, len?: number): argTestDecorator {
    return function (this: { type: argType; len: number }, tagClass: any, fxName: string, descrp: propDescription) {
        let fx = <Function>descrp.value;
        descrp.value = function 参数检查器(this: any) {
...
export function type_debug(Type: argType, ...more: argType[]): argTestDecorator {
    if (ops.debug) {
        try {
...
/** 用来确定检查器 */
function testTarget(argType: argType, args: IArguments, index: number): ReturnType<checker["testFx"]> {
...
const innerChecker: { [key: string]: checker } = {
    ...
    typeof: {
        testFx(this: checker, args: IArguments, index: number) {
            ...
            if (nowType !== type) return { coType: <string>type, nowType, binding: arg };
        },
    },
    instanceof: {
        testFx(args: IArguments, index: number) {
            ...
            if (!(arg instanceof type)) {
                ...
                if (arg === null) nowType = "null";
                    ...
                    nowType = "object " + Reflect.getPrototypeOf(arg)?.constructor.name ?? nowTypeof;
    ...
    array: {
        testFx(args: IArguments, index: number) {
            ...
            this.type = undefined;
            let arg = args[index];
            if (!Array.isArray(arg)) return { coType: "object Array", nowType: typeof arg };
...
export const checkers = {
    ...
    notNullObj: <checker>{
        testFx(args: IArguments, index: number) {
            let arg = args[index];
            if (typeof arg !== "object" || arg === null) return { coType: "class: Array", nowType: typeof arg };
        },
    },
    /** 可枚举类型 */
