...
export function pipeTo<T extends Readable, R extends Writable>(
  ...
  const pms: WithPromise<{ abort(reason?: Error): void }, void> =
    new Promise<void>(function (resolve, reject) {
      ...
      if (!source.readable) {
        const err =
          source.errored instanceof Error
            ? source.errored
            : new Error("Readable is ended");
        return onSourceError(err);
      }
      if (!target.writable) {
        const err =
          target.errored instanceof Error
            ? target.errored
            : new Error("Writable is ended");
        return onTargetError(err);
      }
      function onTargetError(err: Error) {
        clear();
        if (!options.preventReadableDispose && !source.destroyed)
          source.destroy(err);
        reject(new PipeTargetError(err));
      }
      function onSourceError(err: Error) {
        clear();
        if (!options.preventWritableDispose && !target.destroyed)
          target.destroy(err);
  ...
  return new Promise(function (resolve, reject) {
    ...
    Promise.all([a_ctrl, b_ctrl]).then(
      ...
      function (err) {
        ...
        if (err instanceof PipeSourceError || err instanceof PipeTargetError)
          err = err.cause;
        if (!preventDispose) {
