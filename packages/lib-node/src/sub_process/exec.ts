...
function toSpawnOptions(command: string, options: SpawnOptions & { shell?: string }) {
...
export function exec(command: string, options: SpawnOptions & { shell?: string } = {}): Promise<SubProcess> {
    ...
    return rawSpawn(shell, opts, { shell: true }).then((cps) => new SubProcess(cps));
}
/**
 * @public
 * @remark exec 的同步版本
 */
/** @public */
export function execSync(command: string, options: SpawnSyncOptions & { shell?: string } = {}): SpawnSyncResult {
    const [shell, opts] = toSpawnOptions(command, options);
