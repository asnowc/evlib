    ...
    describe("pipeTo", function () {
        ...
        describe("管道异常", async function () {
            ...
            test("中断传输", async function () {
                ...
                await expect(reader.pipeTo(pipeTarget, { preventClose: true, signal: abc.signal })).rejects.toThrow(
                    err
                );
                expect(pipeTarget.close).not.toBeCalled();
            });
    ...
    describe("read", function () {
        describe("指定长度", function () {
            ...
            test("读取到视图", async function () {
                ...
                expect(Buffer.from(view.buffer, view.byteOffset, view.byteLength).toString(), "视图被正确写入").toBe(
                    "abcdef"
                );
