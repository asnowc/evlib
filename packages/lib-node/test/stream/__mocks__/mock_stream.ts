...
export function initOpt(opts: DuplexOpts, construct?: boolean): DuplexMockHd {
    ...
    mockOpts.destroy = vi.fn(opts.destroy ?? ((err, cb) => cb(null))) as StreamMockHd["destroy"];
    mockOpts.read = vi.fn(opts.read ?? ((size) => size));
    mockOpts.write = vi.fn(opts.write ?? ((chunk, ec, cb) => cb(null))) as WritableMockHd["write"];
    mockOpts.final = vi.fn(opts.final ?? ((cb) => cb(null))) as WritableMockHd["final"];
    return mockOpts;
}

export type WritableOpts = NonNullable<ConstructorParameters<typeof Writable>[0]>;
export type ReadableOpts = NonNullable<ConstructorParameters<typeof Readable>[0]>;
