...
import { createReadable, createWritable } from "../../stream/__mocks__/mock_stream.js";
...
test("Readable.toWeb", async function () {
    ...
    stream.push(data);
    await afterTime();
    expect(stream.readableLength, "消费者实际并未读取, 造成 readable 扩大了 highWaterMark").toBe(0);
    stream.push(data);
...
test("Writable.toWeb", async function () {
    ...
    const { mockOpts, stream } = createWritable({ write: mockWrite, highWaterMark: 10 });
    ...
    expect(mockOpts.write, "mockWrite 异步写入,造成 第二次 write时 写入队列后直接 resolve").toBeCalledTimes(1);
    await writer.close();
});
